...
export default class CardUi {
  ...
  drawUi() {
    ...
    this.formEl = this.container.querySelector(
      '[data-widget="creditCard-form-widget"]',
    );
  ...
  clearStatus() {
    ...
    const msFiltered = Array.from(msList).filter(
      (item) => item === 'messageValid' || item === 'messageInvalid',
    );
    ...
    const inFiltered = Array.from(inList).filter(
      (item) => item === 'valid' || item === 'invalid',
    );
